const defaultParams = {
  ...
  targetEdgeCount: 1000, // Based on average expected node degree
  // to achieve perfect game
  targetEdgeCountSigma: 10,
  maxAbsoluteNodeValue: 100, // A hard limit to prevent NaN and Inf
  maxAbsoluteEdgeStrength: 210, // A hard limit to prevent NaN and Inf

  // Learning Parameters
  firingThreshold:1.5, // Amount of input "value current" (1/s)
  // needed to fire
  ...
  survivalReward: 15, // (1/s) the amount of ambient fitness
  ...
  hebbianStrengthFactor: 10, // Modules hebbian reinforcement.
  ...
  nodeInactiveLifetime: 5, // If a node does not fire for N seconds, then it and any edges connected to it (in and out) are removed
  edgeInactiveLifetime: 2.5, // An edge is considered activated when either of its connected nodes fires. If an edge is not active for N seconds, then it is removed
  ...
  edgeSpawnRate: 50, // The average number of edges that will spawn per second in order to get close to `targetEdgeCount`
